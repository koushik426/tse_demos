// Ask SINAI — the custom SINAI AI experience (moved here from the
// Analytics "SINAI AI" modal). Left: the Spotter answer canvas with a
// branded empty-state landing (logo + worksheet sample questions) and its own
// input bar hidden. Right: a "SINAI AI" expert pane that tracks the
// question history and takes follow-ups.
//
// Each question is routed (via routeMessage): general SINAI questions
...
import {
  WORKSHEET_ID,
  SPOTTER_EMBED_FLAGS,
  SINAI_SAMPLE_QUESTIONS,
  SINAI_TRIAL_QUESTIONS,
  SINAI_VIDEO_URL,
...
const WELCOME: Turn = {
  role: 'assistant',
  text: 'Ask me anything about SINAI — carbon management, decarbonization, best practices — or ask about your own data (“emissions by scope”, “top processes by emissions”) and I’ll chart it live.',
};
...
export default function AskSINAI() {
  ...
  const runSpotterQuery = useCallback(
    (query: string) => {
      try {
        ...
        console.warn('[sinai-ai] SpotterSearch failed:', e);
  ...
  async function send(preset?: string) {
    ...
    try {
      ...
      if (result.kind === 'analytics') {
        ...
        // General SINAI question → answered from the knowledge base; Spotter
  ...
  const remaining = Math.max(0, SINAI_TRIAL_QUESTIONS - askedCount);
      ...
      <div className="sl-ai-page">
          ...
          <div className={`sl-ai-canvas${spotterActive ? ' is-spotter-active' : ''}`}>
            {!currentQuery && (
              <div className="sl-ai-empty">
                <SINAILogo className="sl-ai-empty-logo" size={58} wordmark={false} />
                <span className="sl-ai-empty-eyebrow">
                  <Sparkles size={14} /> Powered by SINAI AI
                </span>
                <h2 className="sl-ai-empty-title">Ask SINAI AI</h2>
                <p className="sl-ai-empty-sub">
                  Ask any analytical question about your emissions, processes, activity
                  data, and reduction progress. Answers come straight from your live data
                  with an interactive chart you can drill into.
                </p>
                <div className="sl-ai-empty-chips">
                  {SINAI_SAMPLE_QUESTIONS.map((q) => (
        ...
        <div className="sl-ai-pane">
          ...
          <div className="sl-ai-pane-footer">
            <a href="https://www.sinai.com/legal/privacy-notice" target="_blank" rel="noopener noreferrer">
              Privacy Policy
            </a>
            <span>Powered by SINAI</span>
          </div>
        </div>
      </div>
