...
import { Wand2, Plus, Pin, X, ChevronDown, FileSearch, Calendar, Layers } from 'lucide-react';
import {
...
export default function Analytics() {
  ...
  // Year + Scope multiselects — both empty by default (= no filter / show all).
  const yearRef = useRef<{ column: string; values: string[] }>({ column: '', values: [] });
  const scopeRef = useRef<{ column: string; values: string[] }>({ column: '', values: [] });
  const filtersRef = useRef<any[]>([]);

  function buildFilters(): any[] {
    const f: any[] = [];
    const y = yearRef.current;
    if (y.column && y.values.length)
      f.push({ columnName: y.column, operator: RuntimeFilterOp.IN, values: y.values });
    const s = scopeRef.current;
    if (s.column && s.values.length)
      f.push({ columnName: s.column, operator: RuntimeFilterOp.IN, values: s.values });
    return f;
  }
  ...
  const onYearApply = useCallback((column: string, values: string[]) => {
    yearRef.current = { column, values };
    applyFilters();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  const onScopeApply = useCallback((column: string, values: string[]) => {
    scopeRef.current = { column, values };
    applyFilters();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
      ...
      <div className="analytics-toolbar">
        <div className="analytics-toolbar-left">
          <div>
            ...
            <p className="page-subtitle">Your live emissions performance dashboard</p>
          </div>
          <div className="analytics-filters">
            <ColumnFilter label="Year" icon={Calendar} candidates={YEAR_COLUMN_CANDIDATES} onApply={onYearApply} />
            <ColumnFilter label="Scope" icon={Layers} candidates={SCOPE_COLUMN_CANDIDATES} onApply={onScopeApply} />
          </div>
        </div>
          ...
          <div className="analytics-report-col">
            <div className="report-head">
              <span className="report-title">
                {panel === 'spotter' ? 'SINAI AI' : 'Report Builder'}
              </span>
