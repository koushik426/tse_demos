...
/**
 * Initialise the ThoughtSpot Visual Embed SDK with SINAI styling.
 * Styling (CSS variables + spotter icon) comes straight from sinai_fin.json
...
export function initThoughtSpot(username: string, password: string) {
  ...
  init({
    ...
    customizations: {
      ...
      // Global text overrides — replaces "Spotter" -> "SINAI AI" (and the
...
/**
 * Customizations for the Analytics liveboard — the active-theme SINAI
 ...
 *
 * The viz action buttons (bell / "SINAI AI" / …) read their hover bg from
 ...
 * "My Reports" listing in sinai_fin.json). Sorted most-recently-modified
...
export async function fetchCadences(
  ...
  // when filtered to SINAI_ORG_ID. If none work, fall back to the unfiltered
  // list rather than showing nothing.
  let orgClause = '';
  let orgColumn = '';
  for (const orgCol of ORG_COLUMN_CANDIDATES) {
    const r = await searchData(`[${cadenceCol}] [${orgCol}] = '${SINAI_ORG_ID}'`);
    if (r && r.data_rows?.length) {
      orgClause = ` [${orgCol}] = '${SINAI_ORG_ID}'`;
      orgColumn = orgCol;
  ...
  if (!orgClause) {
    console.warn(
      `[sinai] No org column matched ${SINAI_ORG_ID} (tried: ${ORG_COLUMN_CANDIDATES.join(
        ', ',
